...
public class TasksService :  WorkshopApp.Proto.Client.TasksService.TasksServiceBase
{
    ...
    {
        var comments = await _taskService.GetTaskComments(request.TaskId, context.CancellationToken);

        return new V1GetTaskCommentsResponse
        {
            Comments =
            {
                comments.Select(c => new V1GetTaskCommentsResponse.Types.GetCommentsModel
                {
                    TaskId = c.TaskId,
                    Message = c.Message,
